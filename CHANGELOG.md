...
### Fixed
- `#edit-buttons` CSS 특이도 버그 수정 — ID 셀렉터가 클래스보다 우선하여 수정 전용 버튼이 항상 노출되던 문제
- 모든 버튼 display 토글 `"block"` → `"flex"` 수정 (4개 파일)

### Added (계획)
- Quill 툴바 버튼 아이콘 흰색 처리 (파란 배경 대비 개선)
- 수정완료/취소 버튼 색상 분기 (초록/회색)
- 수정 모드 진입 시 "수정 중" 배지 표시
